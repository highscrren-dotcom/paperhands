...
5. **`StrategyData` (persist snapshot) gained `retryOpenSignal`, `retryOpenCount` and `retryCloseCount`.** Open and close attempts are pre-armed — the counters persist BEFORE each gate call, so `attempt > 0` holds even across a crash mid-attempt (`retryCloseCount` is restored only for its own position, gated by `pendingSignalId` match / a pending user-close drain). On restore both counters are CLAMPED to 1: only the "a prior order may have reached the exchange — reconcile first" bit survives a restart, while the retry budget refreshes — a pre-crash rejection streak can never drop a signal or force-close a position on the first post-restart attempt. Custom `IPersistStrategyInstance` adapters must round-trip the new fields; snapshots written by older versions read back fine (missing fields default to `null`/`0`).
