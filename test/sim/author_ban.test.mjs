...
test("SIM: default-ban — unproven and coin-flipping authors are banned, ban rule swings per grid point", async ({ pass, fail }) => {
  ...
  if (JSON.stringify(result.reports.close.best.find(({ criterion }) => criterion === "sharpe").report.allowedAuthors) !== JSON.stringify(["prophet"])) {
    fail(`allowedAuthors must be ["prophet"], got ${JSON.stringify(result.reports.close.best.find(({ criterion }) => criterion === "sharpe").report.allowedAuthors)}`);
    return;
  }
  if (!result.reports.close.best.find(({ criterion }) => criterion === "sharpe").report.bannedAuthors.includes("loser") || !result.reports.close.best.find(({ criterion }) => criterion === "sharpe").report.bannedAuthors.includes("newbie")) {
    fail(`bannedAuthors must include loser and newbie, got ${JSON.stringify(result.reports.close.best.find(({ criterion }) => criterion === "sharpe").report.bannedAuthors)}`);
    return;
  }
