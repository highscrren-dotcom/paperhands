...
- **Мокать можно прямым присваиванием** (`lib.someService.method = ...`, патч
  ...
  `listenOrderSchedule`) не пересекаются между тестами.
...
- **SL-зеркала интеграционных lifecycle-тестов (LONG и SHORT)**: тот же сценарий пяти механизмов, но после trailingTake рынок разворачивается — закрытие stop_loss РОВНО по подтянутому SL (LONG 47448.98, SHORT 52960.78), причём цена разворота остаётся МЕЖДУ trailed SL и оригинальным (45000/55000 не тронуты — стреляет именно трейлинг); подтянутый TP сохранён в закрытом сигнале, хотя не сработал; те же снапшоты партиалов/entries/invested/коммитов
