...
const fetchData = ttl(async (id: string) => {
  ...
  if (!signalOpenedData) {
    throw new Error("Signal opened data not found");
  }

  if (signalOpenedData.type !== "signal.opened") {
    throw new Error(`Invalid notification data type: expected 'signal.opened', got ${signalOpenedData.type}`);
  }
...
const handleCopy = async (pathname: string, id: string, onCopy: (content: string) => void) => {
  ...
  if (pathname.includes("/signal_opened")) {
    onCopy(JSON.stringify(signal_opened, null, 2));
    return;
  }
...
const handleDownloadPdf = async (id: string) => {
  const { signal_opened } = await fetchData(id);
  if (signal_opened) {
    const content = ioc.markdownHelperService.buildMarkdownFromFields(signal_opened_fields, signal_opened);
...
const handleDownloadMarkdown = async (id: string) => {
  ...
  if (signal_opened) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `signal_opened_${signal_opened.signalId || "unknown"}.md`);
  }
};

export const useSignalOpenedView = () => {
  ...
  const handleTabChange = (id: string, history: History) => {
    if (id === "signal_opened") {
      history.replace(`/signal_opened`);
      setPathname(`/signal_opened`);
    }
    if (id === "candle_1m") {
  ...
  const { pickData, render } = useTabsModal({
    ...
    AfterTitle: ({ onClose }) => (
      <Stack direction="row" gap={1}>
        <Async>
            {async () => {
                const { signal_opened } = await fetchData(id$.current);
                if (!signal_opened) {
                    return null;
                }
                return (
...
export default useSignalOpenedView;
