...
import { OrderFillCloseNotification } from "backtest-kit";
import order_fill_close_fields from "../../assets/order_fill_close_fields";
import MenuIcon from "./components/MenuIcon";
import downloadMarkdown from "../../utils/downloadMarkdown";
import { t } from "../../i18n";

const DEFAULT_PATH = "/order_fill_close";
...
const fetchData = ttl(async (id: string) => {
  ...
  const orderFillCloseData = await ioc.notificationViewService.getOne(id) as OrderFillCloseNotification;

  if (!orderFillCloseData) {
    throw new Error("Order fill close data not found");
  }

  if (orderFillCloseData.type !== "order_fill.close") {
    throw new Error(`Invalid notification data type: expected 'order_fill.close', got ${orderFillCloseData.type}`);
  }

  return {
    order_fill_close: orderFillCloseData,
    candle_1m: await ioc.exchangeViewService.getPointCandles({
      currentTime: orderFillCloseData.timestamp,
      exchangeName: orderFillCloseData.exchangeName,
      interval: "1m",
      symbol: orderFillCloseData.symbol,
    }),
...
const handleDownloadJson = async (pathname: string, id: string) => {
  ...
  const { candle_15m, candle_1h, candle_1m, order_fill_close } = await fetchData(id);

  if (pathname.includes("/order_fill_close")) {
    const blob = new Blob([JSON.stringify(order_fill_close, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `order_fill_close_${order_fill_close.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1m_${order_fill_close.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_15m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_15m_${order_fill_close.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1h")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1h_${order_fill_close.id || "unknown"}.json`);
    return;
  }
};
...
const handleCopy = async (pathname: string, id: string, onCopy: (content: string) => void) => {
  ...
  if (pathname.includes("/order_fill_close")) {
    onCopy(JSON.stringify(order_fill_close, null, 2));
    return;
  }
...
const handleDownloadPdf = async (id: string) => {
  const { order_fill_close } = await fetchData(id);
  if (order_fill_close) {
    const content = ioc.markdownHelperService.buildMarkdownFromFields(order_fill_close_fields, order_fill_close);
...
const handleDownloadMarkdown = async (id: string) => {
  ...
  if (order_fill_close) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `order_fill_close_${order_fill_close.id || "unknown"}.md`);
  }
};

export const useOrderFillCloseView = () => {
  ...
  const handleTabChange = (id: string, history: History) => {
    if (id === "order_fill_close") {
      history.replace(`/order_fill_close`);
      setPathname(`/order_fill_close`);
    }
    if (id === "candle_1m") {
  ...
  const { pickData, render } = useTabsModal({
    ...
    animation: "none",
    title: t("Order Close Fill Details"),
    ...
    AfterTitle: ({ onClose }) => (
      <Stack direction="row" gap={1}>
        <Async>
            {async () => {
                const { order_fill_close } = await fetchData(id$.current);
                if (!order_fill_close) {
                    return null;
                }
                return (
                    ...
                    <ActionIcon
                        ...
                        onClick={() => {
                            ...
                            ioc.routerService.push(
                                `/dump/${order_fill_close.signalId}`,
                            );
                        }}
                    >
                        <Search />
    ...
    mapInitialData: ([{ order_fill_close, ...other }]) => ({
      main: order_fill_close,
      order_fill_close,
      ...other,
    }),
...
export default useOrderFillCloseView;
