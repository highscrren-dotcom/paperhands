...
const fetchData = ttl(async (id: string) => {
  ...
  const partialLossAvailableData = await ioc.notificationViewService.getOne(id) as PartialLossAvailableNotification;

  if (!partialLossAvailableData) {
    throw new Error("Partial loss available data not found");
  }

  if (partialLossAvailableData.type !== "partial_loss.available") {
    throw new Error(`Invalid notification data type: expected 'partial_loss.available', got ${partialLossAvailableData.type}`);
  }

  return {
    partial_loss_available: partialLossAvailableData,
    candle_1m: await ioc.exchangeViewService.getPointCandles({
      currentTime: partialLossAvailableData.timestamp,
      exchangeName: partialLossAvailableData.exchangeName,
      interval: "1m",
      symbol: partialLossAvailableData.symbol,
    }),
...
const handleDownloadJson = async (pathname: string, id: string) => {
  ...
  if (pathname.includes("/partial_loss_available")) {
    const blob = new Blob([JSON.stringify(partial_loss_available, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `partial_loss_available_${partial_loss_available.signalId || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1m_${partial_loss_available.signalId || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_15m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_15m_${partial_loss_available.signalId || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1h")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1h_${partial_loss_available.signalId || "unknown"}.json`);
    return;
  }
};
...
export const usePartialLossAvailableView = () => {
  ...
  const handleTabChange = (id: string, history: History) => {
    if (id === "partial_loss_available") {
      history.replace(`/partial_loss_available`);
      setPathname(`/partial_loss_available`);
    }
    if (id === "candle_1m") {
  ...
  const { pickData, render } = useTabsModal({
    ...
    animation: "none",
    title: t("Partial Loss Available Details"),
    ...
    AfterTitle: ({ onClose }) => (
      <Stack direction="row" gap={1}>
        ...
        <Async>
          {async () => {
              const { partial_loss_available } = await fetchData(id$.current);
              if (!partial_loss_available?.signalId) {
                  return null;
              }
              return (
                  <ActionIcon
                      ...
                      onClick={() => {
                          ...
                          ioc.routerService.push(
                              `/dump/${partial_loss_available.signalId}`,
                          );
                      }}
                  >
                      <Search />
    ...
    mapInitialData: ([{ partial_loss_available, ...other }]) => ({
      main: partial_loss_available,
      partial_loss_available,
      ...other,
    }),
  ...
  return (id: string, route = DEFAULT_PATH) => {
    push({
      id: "partial_loss_available_modal",
      render,
      onInit: () => {
        history.push(route);
        setPathname(route);
...
export default usePartialLossAvailableView;
