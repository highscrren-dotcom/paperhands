...
const fetchData = ttl(async (id: string) => {
  ...
  const partialProfitAvailableData = await ioc.notificationViewService.getOne(id) as PartialProfitAvailableNotification;

  if (!partialProfitAvailableData) {
    throw new Error("Partial profit available data not found");
  }

  if (partialProfitAvailableData.type !== "partial_profit.available") {
    throw new Error(`Invalid notification data type: expected 'partial_profit.available', got ${partialProfitAvailableData.type}`);
  }

  return {
    partial_profit_available: partialProfitAvailableData,
    candle_1m: await ioc.exchangeViewService.getPointCandles({
      currentTime: partialProfitAvailableData.timestamp,
      exchangeName: partialProfitAvailableData.exchangeName,
      interval: "1m",
      symbol: partialProfitAvailableData.symbol,
    }),
...
const handleDownloadJson = async (pathname: string, id: string) => {
  ...
  const { candle_15m, candle_1h, candle_1m, partial_profit_available } = await fetchData(id);

  if (pathname.includes("/partial_profit_available")) {
    const blob = new Blob([JSON.stringify(partial_profit_available, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `partial_profit_available_${partial_profit_available.signalId || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1m_${partial_profit_available.signalId || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_15m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_15m_${partial_profit_available.signalId || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1h")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1h_${partial_profit_available.signalId || "unknown"}.json`);
    return;
  }
};
...
const handleCopy = async (pathname: string, id: string, onCopy: (content: string) => void) => {
  ...
  if (pathname.includes("/partial_profit_available")) {
    onCopy(JSON.stringify(partial_profit_available, null, 2));
    return;
  }
...
const handleDownloadPdf = async (id: string) => {
  const { partial_profit_available } = await fetchData(id);
  if (partial_profit_available) {
    const content = ioc.markdownHelperService.buildMarkdownFromFields(partial_profit_available_fields, partial_profit_available);
...
const handleDownloadMarkdown = async (id: string) => {
  ...
  if (partial_profit_available) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `partial_profit_available_${partial_profit_available.signalId || "unknown"}.md`);
  }
};

export const usePartialProfitAvailableView = () => {
  ...
  const handleTabChange = (id: string, history: History) => {
    if (id === "partial_profit_available") {
      history.replace(`/partial_profit_available`);
      setPathname(`/partial_profit_available`);
    }
    if (id === "candle_1m") {
  ...
  const { pickData, render } = useTabsModal({
    ...
    mapInitialData: ([{ partial_profit_available, ...other }]) => ({
      main: partial_profit_available,
      partial_profit_available,
      ...other,
    }),
  ...
  return (id: string, route = DEFAULT_PATH) => {
    push({
      id: "partial_profit_available_modal",
      render,
      onInit: () => {
        history.push(route);
        setPathname(route);
...
export default usePartialProfitAvailableView;
