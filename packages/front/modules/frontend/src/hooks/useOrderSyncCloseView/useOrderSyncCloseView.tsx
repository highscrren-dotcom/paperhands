...
import { OrderSyncCloseNotification } from "backtest-kit";
...
const fetchData = ttl(async (id: string) => {
  ...
  const orderSyncCloseData = await ioc.notificationViewService.getOne(id) as OrderSyncCloseNotification;

  if (!orderSyncCloseData) {
    throw new Error("Order sync close data not found");
  }

  if (orderSyncCloseData.type !== "order_sync.close") {
    throw new Error(`Invalid notification data type: expected 'order_sync.close', got ${orderSyncCloseData.type}`);
  }

  return {
    order_sync_close: orderSyncCloseData,
    candle_1m: await ioc.exchangeViewService.getPointCandles({
      currentTime: orderSyncCloseData.timestamp,
      exchangeName: orderSyncCloseData.exchangeName,
      interval: "1m",
      symbol: orderSyncCloseData.symbol,
    }),
...
const handleDownloadJson = async (pathname: string, id: string) => {
  ...
  const { candle_15m, candle_1h, candle_1m, order_sync_close } = await fetchData(id);

  if (pathname.includes("/order_sync_close")) {
    const blob = new Blob([JSON.stringify(order_sync_close, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `order_sync_close_${order_sync_close.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1m_${order_sync_close.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_15m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_15m_${order_sync_close.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1h")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1h_${order_sync_close.id || "unknown"}.json`);
    return;
  }
};
...
const handleCopy = async (pathname: string, id: string, onCopy: (content: string) => void) => {
  ...
  if (pathname.includes("/order_sync_close")) {
    onCopy(JSON.stringify(order_sync_close, null, 2));
    return;
  }
...
const handleDownloadPdf = async (id: string) => {
  const { order_sync_close } = await fetchData(id);
  if (order_sync_close) {
    const content = ioc.markdownHelperService.buildMarkdownFromFields(order_sync_close_fields, order_sync_close);
...
const handleDownloadMarkdown = async (id: string) => {
  ...
  if (order_sync_close) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `order_sync_close_${order_sync_close.id || "unknown"}.md`);
  }
};

export const useOrderSyncCloseView = () => {
  ...
  const handleTabChange = (id: string, history: History) => {
    if (id === "order_sync_close") {
      history.replace(`/order_sync_close`);
      setPathname(`/order_sync_close`);
    }
    if (id === "candle_1m") {
  ...
  const { pickData, render } = useTabsModal({
    ...
    AfterTitle: ({ onClose }) => (
      <Stack direction="row" gap={1}>
        <Async>
            {async () => {
                const { order_sync_close } = await fetchData(id$.current);
                if (!order_sync_close) {
                    return null;
                }
                return (
                    ...
                    <ActionIcon
                        ...
                        onClick={() => {
                            ...
                            ioc.routerService.push(
                                `/dump/${order_sync_close.signalId}`,
                            );
                        }}
                    >
                        <Search />
    ...
    mapInitialData: ([{ order_sync_close, ...other }]) => ({
      main: order_sync_close,
      order_sync_close,
      ...other,
    }),
...
export default useOrderSyncCloseView;
