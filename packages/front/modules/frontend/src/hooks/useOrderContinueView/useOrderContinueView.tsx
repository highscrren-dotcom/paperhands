...
import { OrderContinueCheckNotification } from "backtest-kit";
import order_continue_fields from "../../assets/order_continue_fields";
import MenuIcon from "./components/MenuIcon";
import downloadMarkdown from "../../utils/downloadMarkdown";
import { t } from "../../i18n";

const DEFAULT_PATH = "/order_continue";
...
const fetchData = ttl(async (id: string) => {
  ...
  const orderContinueData = await ioc.notificationViewService.getOne(id) as OrderContinueCheckNotification;

  if (!orderContinueData) {
    throw new Error("Order continue data not found");
  }

  if (orderContinueData.type !== "order_continue.check") {
    throw new Error(`Invalid notification data type: expected 'order_continue.check', got ${orderContinueData.type}`);
  }

  return {
    order_continue: orderContinueData,
    candle_1m: await ioc.exchangeViewService.getPointCandles({
      currentTime: orderContinueData.timestamp,
      exchangeName: orderContinueData.exchangeName,
      interval: "1m",
      symbol: orderContinueData.symbol,
    }),
...
const handleDownloadJson = async (pathname: string, id: string) => {
  ...
  const { candle_15m, candle_1h, candle_1m, order_continue } = await fetchData(id);

  if (pathname.includes("/order_continue")) {
    const blob = new Blob([JSON.stringify(order_continue, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `order_continue_${order_continue.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1m_${order_continue.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_15m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_15m_${order_continue.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1h")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1h_${order_continue.id || "unknown"}.json`);
    return;
  }
};
...
const handleCopy = async (pathname: string, id: string, onCopy: (content: string) => void) => {
  ...
  if (pathname.includes("/order_continue")) {
    onCopy(JSON.stringify(order_continue, null, 2));
    return;
  }
...
const handleDownloadPdf = async (id: string) => {
  const { order_continue } = await fetchData(id);
  if (order_continue) {
    const content = ioc.markdownHelperService.buildMarkdownFromFields(order_continue_fields, order_continue);
...
const handleDownloadMarkdown = async (id: string) => {
  ...
  if (order_continue) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `order_continue_${order_continue.id || "unknown"}.md`);
  }
};

export const useOrderContinueView = () => {
  ...
  const handleTabChange = (id: string, history: History) => {
    if (id === "order_continue") {
      history.replace(`/order_continue`);
      setPathname(`/order_continue`);
    }
    if (id === "candle_1m") {
  ...
  const { pickData, render } = useTabsModal({
    ...
    animation: "none",
    title: t("Order Continue Details"),
    ...
    AfterTitle: ({ onClose }) => (
      <Stack direction="row" gap={1}>
        <Async>
            {async () => {
                const { order_continue } = await fetchData(id$.current);
                if (!order_continue) {
                    return null;
                }
                return (
    ...
    mapInitialData: ([{ order_continue, ...other }]) => ({
      main: order_continue,
      order_continue,
      ...other,
    }),
...
export default useOrderContinueView;
