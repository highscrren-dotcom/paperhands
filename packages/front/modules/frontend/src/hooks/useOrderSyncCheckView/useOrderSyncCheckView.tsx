...
import { OrderSyncCheckNotification } from "backtest-kit";
...
const fetchData = ttl(async (id: string) => {
  ...
  const orderSyncCheckData = await ioc.notificationViewService.getOne(id) as OrderSyncCheckNotification;

  if (!orderSyncCheckData) {
    throw new Error("Order sync check data not found");
  }

  if (orderSyncCheckData.type !== "order_sync.check") {
    throw new Error(`Invalid notification data type: expected 'order_sync.check', got ${orderSyncCheckData.type}`);
  }

  return {
    order_sync_check: orderSyncCheckData,
    candle_1m: await ioc.exchangeViewService.getPointCandles({
      currentTime: orderSyncCheckData.timestamp,
      exchangeName: orderSyncCheckData.exchangeName,
      interval: "1m",
      symbol: orderSyncCheckData.symbol,
    }),
...
const handleDownloadJson = async (pathname: string, id: string) => {
  ...
  const { candle_15m, candle_1h, candle_1m, order_sync_check } = await fetchData(id);

  if (pathname.includes("/order_sync_check")) {
    const blob = new Blob([JSON.stringify(order_sync_check, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `order_sync_check_${order_sync_check.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1m_${order_sync_check.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_15m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_15m_${order_sync_check.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1h")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1h_${order_sync_check.id || "unknown"}.json`);
    return;
  }
};
...
const handleCopy = async (pathname: string, id: string, onCopy: (content: string) => void) => {
  ...
  if (pathname.includes("/order_sync_check")) {
    onCopy(JSON.stringify(order_sync_check, null, 2));
    return;
  }
...
const handleDownloadPdf = async (id: string) => {
  const { order_sync_check } = await fetchData(id);
  if (order_sync_check) {
    const content = ioc.markdownHelperService.buildMarkdownFromFields(order_sync_check_fields, order_sync_check);
...
const handleDownloadMarkdown = async (id: string) => {
  ...
  if (order_sync_check) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `order_sync_check_${order_sync_check.id || "unknown"}.md`);
  }
};

export const useOrderSyncCheckView = () => {
  ...
  const handleTabChange = (id: string, history: History) => {
    if (id === "order_sync_check") {
      history.replace(`/order_sync_check`);
      setPathname(`/order_sync_check`);
    }
    if (id === "candle_1m") {
  ...
  const { pickData, render } = useTabsModal({
    ...
    AfterTitle: ({ onClose }) => (
      <Stack direction="row" gap={1}>
        <Async>
            {async () => {
                const { order_sync_check } = await fetchData(id$.current);
                if (!order_sync_check) {
                    return null;
                }
                return (
                    ...
                    <ActionIcon
                        ...
                        onClick={() => {
                            ...
                            ioc.routerService.push(
                                `/dump/${order_sync_check.signalId}`,
                            );
                        }}
                    >
                        <Search />
    ...
    mapInitialData: ([{ order_sync_check, ...other }]) => ({
      main: order_sync_check,
      order_sync_check,
      ...other,
    }),
...
export default useOrderSyncCheckView;
