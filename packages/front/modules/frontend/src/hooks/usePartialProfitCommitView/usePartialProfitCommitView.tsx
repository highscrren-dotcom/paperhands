import {
  useActualState,
  useModalManager,
  useTabsModal,
  History,
  useActualRef,
  ActionIcon,
  Async,
} from "react-declarative";
import { ttl } from "../../utils/ttl";
import { ArrowBack, Close, Print, Search } from "@mui/icons-material";
import { createMemoryHistory } from "history";
import routes from "./routes";
import { CC_FULLSCREEN_SIZE_REQUEST } from "../../config/params";
import tabs from "./tabs";
import { Box, Stack } from "@mui/material";
import ioc from "../../lib";
import CopyIcon from "./components/CopyIcon";
import { PartialProfitCommitNotification } from "backtest-kit";
import partial_profit_commit_fields from "../../assets/partial_profit_commit_fields";
import MenuIcon from "./components/MenuIcon";
import downloadMarkdown from "../../utils/downloadMarkdown";
import { t } from "../../i18n";

const DEFAULT_PATH = "/partial_profit_commit";
const CACHE_TTL = 45_000;

const history = createMemoryHistory();

const fetchData = ttl(async (id: string) => {

  const partialProfitCommitData = await ioc.notificationViewService.getOne(id) as PartialProfitCommitNotification;

  if (!partialProfitCommitData) {
    throw new Error("Partial profit commit data not found");
  }

  if (partialProfitCommitData.type !== "partial_profit.commit") {
    throw new Error(`Invalid notification data type: expected 'partial_profit.commit', got ${partialProfitCommitData.type}`);
  }

  return {
    partial_profit_commit: partialProfitCommitData,
    candle_1m: await ioc.exchangeViewService.getPointCandles({
      currentTime: partialProfitCommitData.timestamp,
      exchangeName: partialProfitCommitData.exchangeName,
      interval: "1m",
      symbol: partialProfitCommitData.symbol,
    }),
    candle_15m: await ioc.exchangeViewService.getPointCandles({
      currentTime: partialProfitCommitData.timestamp,
      exchangeName: partialProfitCommitData.exchangeName,
      interval: "15m",
      symbol: partialProfitCommitData.symbol,
    }),
    candle_1h: await ioc.exchangeViewService.getPointCandles({
      currentTime: partialProfitCommitData.timestamp,
      exchangeName: partialProfitCommitData.exchangeName,
      interval: "1h",
      symbol: partialProfitCommitData.symbol,
    }),
  };
}, {
  timeout: CACHE_TTL,
  key: ([id]) => `${id}`,
});

const handleDownloadJson = async (pathname: string, id: string) => {

  const { candle_15m, candle_1h, candle_1m, partial_profit_commit } = await fetchData(id);

  if (pathname.includes("/partial_profit_commit")) {
    const blob = new Blob([JSON.stringify(partial_profit_commit, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `partial_profit_commit_${partial_profit_commit.id || "unknown"}.json`);
    return;
  }

  if (pathname.includes("/candle_1m")) {
    const blob = new Blob([JSON.stringify(candle_1m, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1m_${partial_profit_commit.id || "unknown"}.json`);
    return;
  }

  if (pathname.includes("/candle_15m")) {
    const blob = new Blob([JSON.stringify(candle_15m, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_15m_${partial_profit_commit.id || "unknown"}.json`);
    return;
  }

  if (pathname.includes("/candle_1h")) {
    const blob = new Blob([JSON.stringify(candle_1h, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1h_${partial_profit_commit.id || "unknown"}.json`);
    return;
  }
};

const handleCopy = async (pathname: string, id: string, onCopy: (content: string) => void) => {
  const { candle_15m, candle_1h, candle_1m, partial_profit_commit } = await fetchData(id);

  if (pathname.includes("/partial_profit_commit")) {
    onCopy(JSON.stringify(partial_profit_commit, null, 2));
    return;
  }

  if (pathname.includes("/candle_1m")) {
    onCopy(JSON.stringify(candle_1m, null, 2));
    return;
  }

  if (pathname.includes("/candle_15m")) {
    onCopy(JSON.stringify(candle_15m, null, 2));
    return;
  }

  if (pathname.includes("/candle_1h")) {
    onCopy(JSON.stringify(candle_1h, null, 2));
    return;
  }
};

const handleDownloadPdf = async (id: string) => {
  const { partial_profit_commit } = await fetchData(id);
  if (partial_profit_commit) {
    const content = ioc.markdownHelperService.buildMarkdownFromFields(partial_profit_commit_fields, partial_profit_commit);
    await downloadMarkdown(content);
  }
};

const handleDownloadMarkdown = async (id: string) => {
  const { partial_profit_commit } = await fetchData(id);
  if (partial_profit_commit) {
    const content = ioc.markdownHelperService.buildMarkdownFromFields(partial_profit_commit_fields, partial_profit_commit);
    const blob = new Blob([content], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `partial_profit_commit_${partial_profit_commit.id || "unknown"}.md`);
  }
};

export const usePartialProfitCommitView = () => {

  const [id$, setId] = useActualState("");
  const ctx = useModalManager();
  const { push, pop } = ctx;

  const [pathname$, setPathname] = useActualRef(history.location.pathname);

  const handleTabChange = (id: string, history: History) => {
    if (id === "partial_profit_commit") {
      history.replace(`/partial_profit_commit`);
      setPathname(`/partial_profit_commit`);
    }
    if (id === "candle_1m") {
      history.replace(`/candle_1m`);
      setPathname(`/candle_1m`);
    }
    if (id === "candle_15m") {
      history.replace(`/candle_15m`);
      setPathname(`/candle_15m`);
    }
    if (id === "candle_1h") {
      history.replace(`/candle_1h`);
      setPathname(`/candle_1h`);
    }
  };

  const { pickData, render } = useTabsModal({
    tabs,
    withStaticAction: true,
    onTabChange: handleTabChange,
    animation: "none",
    title: t("Partial Profit Commit Details"),
    sizeRequest: CC_FULLSCREEN_SIZE_REQUEST,
    history,
    routes,
    BeforeTitle: ({ onClose }) => {
      const { total } = useModalManager();
      return (
        <Box
          sx={{
            mr: 1,
            display: total === 1 ? "none" : "flex",
          }}
        >
          <ActionIcon onClick={onClose}>
            <ArrowBack />
          </ActionIcon>
        </Box>
      );
    },
    AfterTitle: ({ onClose }) => (
      <Stack direction="row" gap={1}>
        <Async>
            {async () => {
                const { partial_profit_commit } = await fetchData(id$.current);
                if (!partial_profit_commit) {
                    return null;
                }
                return (
                    <ActionIcon
                        sx={{ display: { xs: "none", sm: "flex" }, mr: "10px" }}
                        onClick={() => ioc.markdownHelperService.printFields(
                            partial_profit_commit_fields,
                            partial_profit_commit,
                        )}
                    >
                        <Print />
                    </ActionIcon>
                );
            }}
        </Async>
        <Async>
            {async () => {
                const { partial_profit_commit } = await fetchData(id$.current);
                if (!partial_profit_commit?.signalId) {
                    return null;
                }
                return (
                    <ActionIcon
                        sx={{ display: { xs: "none", sm: "flex" }, mr: "10px" }}
                        onClick={() => {
                            ctx.clear();
                            ioc.routerService.push(
                                `/dump/${partial_profit_commit.signalId}`,
                            );
                        }}
                    >
                        <Search />
                    </ActionIcon>
                );
            }}
        </Async>
        <CopyIcon
          onClick={async (_, onCopy) => {
            await handleCopy(pathname$.current, id$.current, onCopy)
          }}
          sx={{ display: { xs: "none", sm: "flex" }, mr: "10px", mt: "2.5px" }}
        />
        <MenuIcon
          sx={{ mr: "10px", mt: "0.5px" }}
          onDownloadJson={() => handleDownloadJson(pathname$.current, id$.current)}
          onDownloadMarkdown={() => handleDownloadMarkdown(id$.current)}
          onDownloadPdf={() => handleDownloadPdf(id$.current)}
        />
        <ActionIcon onClick={onClose}>
          <Close />
        </ActionIcon>
      </Stack>
    ),
    fetchState: async () => await fetchData(id$.current),
    mapInitialData: ([{ partial_profit_commit, ...other }]) => ({
      main: partial_profit_commit,
      partial_profit_commit,
      ...other,
    }),
    mapPayload: ([{ candle_1m = [], candle_15m = [], candle_1h = [] }]) => {
      return {
        candle_1m,
        candle_15m,
        candle_1h,
      };
    },
    onLoadStart: () => ioc.layoutService.setAppbarLoader(true),
    onLoadEnd: () => ioc.layoutService.setAppbarLoader(false),
    onClose: () => {
      pop();
    },
  });

  return (id: string, route = DEFAULT_PATH) => {
    push({
      id: "partial_profit_commit_modal",
      render,
      onInit: () => {
        history.push(route);
        setPathname(route);
      },
      onMount: () => {
        setId(id);
        pickData();
      },
    });
  };
};

export default usePartialProfitCommitView;
