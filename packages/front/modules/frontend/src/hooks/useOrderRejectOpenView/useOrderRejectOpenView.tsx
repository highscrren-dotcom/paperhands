...
import { OrderRejectOpenNotification } from "backtest-kit";
import order_reject_open_fields from "../../assets/order_reject_open_fields";
import MenuIcon from "./components/MenuIcon";
import downloadMarkdown from "../../utils/downloadMarkdown";
import { t } from "../../i18n";

const DEFAULT_PATH = "/order_reject_open";
...
const fetchData = ttl(async (id: string) => {
  ...
  const orderRejectOpenData = await ioc.notificationViewService.getOne(id) as OrderRejectOpenNotification;

  if (!orderRejectOpenData) {
    throw new Error("Order reject open data not found");
  }

  if (orderRejectOpenData.type !== "order_reject.open") {
    throw new Error(`Invalid notification data type: expected 'order_reject.open', got ${orderRejectOpenData.type}`);
  }

  return {
    order_reject_open: orderRejectOpenData,
    candle_1m: await ioc.exchangeViewService.getPointCandles({
      currentTime: orderRejectOpenData.timestamp,
      exchangeName: orderRejectOpenData.exchangeName,
      interval: "1m",
      symbol: orderRejectOpenData.symbol,
    }),
...
const handleDownloadJson = async (pathname: string, id: string) => {
  ...
  const { candle_15m, candle_1h, candle_1m, order_reject_open } = await fetchData(id);

  if (pathname.includes("/order_reject_open")) {
    const blob = new Blob([JSON.stringify(order_reject_open, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `order_reject_open_${order_reject_open.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1m_${order_reject_open.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_15m")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_15m_${order_reject_open.id || "unknown"}.json`);
    return;
  }
  ...
  if (pathname.includes("/candle_1h")) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `candles_1h_${order_reject_open.id || "unknown"}.json`);
    return;
  }
};
...
const handleCopy = async (pathname: string, id: string, onCopy: (content: string) => void) => {
  ...
  if (pathname.includes("/order_reject_open")) {
    onCopy(JSON.stringify(order_reject_open, null, 2));
    return;
  }
...
const handleDownloadPdf = async (id: string) => {
  const { order_reject_open } = await fetchData(id);
  if (order_reject_open) {
    const content = ioc.markdownHelperService.buildMarkdownFromFields(order_reject_open_fields, order_reject_open);
...
const handleDownloadMarkdown = async (id: string) => {
  ...
  if (order_reject_open) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `order_reject_open_${order_reject_open.id || "unknown"}.md`);
  }
};

export const useOrderRejectOpenView = () => {
  ...
  const handleTabChange = (id: string, history: History) => {
    if (id === "order_reject_open") {
      history.replace(`/order_reject_open`);
      setPathname(`/order_reject_open`);
    }
    if (id === "candle_1m") {
  ...
  const { pickData, render } = useTabsModal({
    ...
    animation: "none",
    title: t("Order Open Rejected Details"),
    ...
    AfterTitle: ({ onClose }) => (
      <Stack direction="row" gap={1}>
        <Async>
            {async () => {
                const { order_reject_open } = await fetchData(id$.current);
                if (!order_reject_open) {
                    return null;
                }
                return (
                    ...
                    <ActionIcon
                        ...
                        onClick={() => {
                            ...
                            ioc.routerService.push(
                                `/dump/${order_reject_open.signalId}`,
                            );
                        }}
                    >
                        <Search />
    ...
    mapInitialData: ([{ order_reject_open, ...other }]) => ({
      main: order_reject_open,
      order_reject_open,
      ...other,
    }),
...
export default useOrderRejectOpenView;
