...
const handleDownloadPdf = async (id: string) => {
    const { signal } = await fetchData(id);
    if (signal) {
        const content = ioc.markdownHelperService.buildMarkdownFromFields(signal_fields, signal);
...
const handleDownloadMarkdown = async (id: string) => {
    ...
    if (signal) {
        ...
        const url = URL.createObjectURL(blob);
        ioc.layoutService.downloadFile(url, `signal_${signal.id || "unknown"}.md`);
    }
};
