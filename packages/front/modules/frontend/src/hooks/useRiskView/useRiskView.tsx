...
const fetchData = ttl(async (id: string) => {
  ...
  const riskData = await ioc.notificationViewService.getOne(id) as RiskRejectionNotification;

  if (!riskData) {
    throw new Error("Risk data not found");
  }

  if (riskData.type !== "risk.rejection") {
    throw new Error(`Invalid notification data type: expected 'risk.rejection', got ${riskData.type}`);
  }

  return {
    risk: riskData,
    candle_1m: await ioc.exchangeViewService.getPointCandles({
      currentTime: riskData.timestamp,
      exchangeName: riskData.exchangeName,
      interval: "1m",
...
const handleDownloadJson = async (pathname: string, id: string) => {
  ...
  const { candle_15m, candle_1h, candle_1m, risk } = await fetchData(id);
...
const handleCopy = async (pathname: string, id: string, onCopy: (content: string) => void) => {
  ...
  if (pathname.includes("/risk")) {
    onCopy(JSON.stringify(risk, null, 2));
    return;
  }
