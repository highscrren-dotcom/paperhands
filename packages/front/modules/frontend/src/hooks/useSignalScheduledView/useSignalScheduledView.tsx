...
const handleCopy = async (pathname: string, id: string, onCopy: (content: string) => void) => {
  ...
  if (pathname.includes("/signal_scheduled")) {
    onCopy(JSON.stringify(signal_scheduled, null, 2));
    return;
  }
...
const handleDownloadPdf = async (id: string) => {
  const { signal_scheduled } = await fetchData(id);
  if (signal_scheduled) {
    const content = ioc.markdownHelperService.buildMarkdownFromFields(signal_scheduled_fields, signal_scheduled);
...
const handleDownloadMarkdown = async (id: string) => {
  ...
  if (signal_scheduled) {
    ...
    const url = URL.createObjectURL(blob);
    ioc.layoutService.downloadFile(url, `signal_scheduled_${signal_scheduled.signalId || "unknown"}.md`);
  }
};
  ...
  const { pickData, render } = useTabsModal({
    ...
    AfterTitle: ({ onClose }) => (
      <Stack direction="row" gap={1}>
        <Async>
            {async () => {
                const { signal_scheduled } = await fetchData(id$.current);
                if (!signal_scheduled) {
                    return null;
                }
                return (
                    ...
                    <ActionIcon
                        ...
                        onClick={() => {
                            ...
                            ioc.routerService.push(
                                `/dump/${signal_scheduled.signalId}`,
                            );
                        }}
                    >
                        <Search />
