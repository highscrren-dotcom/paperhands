// Фаза A — узкая окрестность за краем сетки (450 точек).
...
import { addExchangeSchema, addSimulatorSchema, Simulator, setConfig } from "backtest-kit";
...
const result = await Simulator.run({ symbol: SYMBOL, simulatorName: "tv_edge", ideas });
flush();
