...
> **СТАТУС 2026-07-08 (session 8): LIVE ЗАПУЩЕН** — решением владельца, с
> осознанным отступлением от части пунктов ниже. Не paperhands/feb, а
> **jan_2026 в форке backtest-ollama-crontab** (spot, long-only, $100).
> Закрыто: live.module + **дословный авторский Spot-адаптер** (SL/TP реальными
> ордерами на бирже), Binance-ключи (spot✓/withdraw✗), préflight-скрипт,
> Telegram-алерты (бот→группа), @reboot-автостарт, дрейф-алерт. Осознанно
> отложено/принято: VPS (локальный хост), IP-whitelist (авто-отзыв ~90 дней),
> авторский гейт certified:false (малый сайз), funding/spot-шорты (шорты
> отфильтрованы), addSizing/addRisk (дефолт $100/сделку, одна позиция при
> текущем балансе). Разделы B (VPS) и C (порядок) остаются планом для
> масштабирования. Хроника — DECISIONS №44-46.
