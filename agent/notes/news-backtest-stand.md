...
1. **[ПЕРЕОБЪЯСНЁН 18.07, старая трактовка неверна] Фантомная 9-я сделка в
   ...
   **шортовый SL −1.4034044044…** (совпадение до 10 знака, wall-clock MTM так
   ...
   снята. Гипотеза механизма (не проверена): каждая запись в dump/log/log.jsonl
   пишется дважды с одним id — если статс-редьюсер ест тот же стрим без дедупа,
   один close попадает в агрегат дважды. **Вопрос автору подготовлен.**
