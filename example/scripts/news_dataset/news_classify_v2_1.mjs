// news_classify_v2_1.mjs — ЭКСПЕРИМЕНТ (vibe-mining): v2.1 = v2 с двумя правками
// по итогам A/B v1↔v2 (см. report.md): (1) TA-опинионка с явным зовом направления —
// это direction с НИЗКИМ confidence (~0.3), а не null (v2-якорь «pure TA → null»
// голодоморил топ-канал kitco; редакторский суд о ценности TA-статей — работа
// pump-anomaly/channelScore, не классификатора); (2) «суди ГЛАВНУЮ историю статьи
// (заголовок), не второстепенные факты из выдержки» (v2 схватил Strategy-продажу
// внутри статьи про Trump-отскок и перевернул direction).
// БОЕВОЙ news_classify.mjs НЕ ТРОГАЕТ: читает тот же news-raw.jsonl, пишет в
// agent/notes/vibe-mining/news-classified-v2_1.jsonl. Паттерн вызова идентичен v1/v2.
// Инвариант: классификатор видит ТОЛЬКО title+content — ни цены, ни даты.
//
// Usage: node news_classify_v2_1.mjs
...
const CLS_PATH = new URL("news-classified-v2_1.jsonl", OUT_DIR);
