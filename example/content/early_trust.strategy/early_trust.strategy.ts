 ...
 *
 * Правило, прошедшее контроли (walk-forward, 1000 жеребьёвок): топ-K авторов по
 * доле ранних постов (<= 2 чужих поста по символу за 24 ч до публикации) за
 * скользящее окно. Окно здесь 12 МЕСЯЦЕВ, K=2 — исходная валидированная ячейка
 * (+3.06 %/сделку против поля +0.31, p = 0.005). Окно 60 мес статистически
 * сильнее (p = 0.001), но непригодно для live: чистая ранность 60-месячного
 * окна выводит в топ авторов, УШЕДШИХ с платформы в 2023 (Avramis/Solldy/
 * drcrypto14 молчат 2+ года) — стенд молчал бы вечно. Допуск >= 10 идей за
 * 12 мес сам гарантирует живых. Оговорка 12-мес ячейки: 2025 год в минусе
 * (-1.37), это часть того, что paper и проверяет.
 *
