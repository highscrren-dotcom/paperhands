...
- **[ai-trading-mcp](https://github.com/backtest-kit/ai-trading-mcp)** — an AI news-trading rig on `@backtest-kit/mcp`: a GramJS scraper pipes a live Telegram channel — text **and chart screenshots** — into the `get_status` feed as MCP image blocks, Claude decides when and which way through the 3 guarded tools, every byte the model saw is dumped to markdown for audit, and a battle-hardened Binance spot broker adapter (OCO brackets, idempotent recovery by `clientOrderId`, verified close) takes it live. The strategy file registers no entry logic at all — the LLM is the only signal source, the engine is the only executor.
